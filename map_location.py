...
async def _send_rich(deps, bot, chat_id: int, subtitle: str, body: str):
    return await bot.send_rich_message(
        chat_id=chat_id,
        rich_message=deps.build_rich_message(_frame(subtitle, body)),
        request_timeout=60,
    )


async def api_map_request(deps, request: web.Request):
    try:
        data = await request.json()
    except Exception:
        return web.json_response({"ok": False, "error": "JSON inválido"}, status=400)
    user, error = _auth(deps, request, data)
    if error:
        return error
    ...
    try:
        ...
        deps.log.exception("api_map_request")
...
async def api_map_status(deps, request: web.Request):
    try:
        data = await request.json()
    except Exception:
        return web.json_response({"ok": False, "error": "JSON inválido"}, status=400)
...
async def api_map_send_location(deps, request: web.Request):
    try:
        data = await request.json()
    except Exception:
        return web.json_response({"ok": False, "error": "JSON inválido"}, status=400)
    ...
    try:
        ...
        deps.log.exception("api_map_send_location")
        return web.json_response({"ok": False, "error": "Não foi possível enviar a localização na DM."}, status=502)
    return web.json_response({"ok": True})


async def handle_location(deps, message, bot) -> None:
    if message.chat.type != deps.ChatType.PRIVATE or not message.from_user:
        return
    _purge()
    ...
    try:
        await _send_rich(deps, bot, chat_id, "Mapa", "Localização recebida.")
    except Exception:
        deps.log.exception("map_location_confirmation")
