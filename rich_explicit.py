"""Compila projeções Rich explícitas usando um único plano estrutural."""
...
_MEDIA_URI = re.compile(
    r"^tg://(photo|video|document|audio)\?id=([A-Za-z0-9_-]{1,64})$", re.I
)
_EXPLICIT_BLOCK_TAGS = frozenset(
    {
        "p", "h1", "h2", "h3", "h4", "h5", "h6", "footer", "hr", "pre",
        "blockquote", "aside", "details", "ul", "ol", "table", "figure",
        "img", "video", "audio", "tg-document", "tg-map", "tg-math-block",
        "tg-button-row", "tg-collage", "tg-slideshow", "b", "strong", "i",
        "em", "u", "ins", "s", "strike", "del", "code", "mark", "sub",
        "sup", "a", "tg-spoiler", "tg-emoji", "tg-time", "tg-math",
        "tg-reference", "tg-entity", "tg-button",
    }
)
...
class _TreeParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs) -> None:
        node = _Node(
            tag.lower(),
            {
                name.lower(): (value if value is not None else True)
...
class ExplicitBlockPlan:
    """Decisão parseada uma vez e compartilhada por roteamento e compilação."""
...
def _explicit_reasons(root: _Node) -> tuple[str, ...]:
    found: list[str] = []
    seen: set[str] = set()

    def visit(node: _Node) -> None:
        if node.tag in _EXPLICIT_BLOCK_TAGS and node.tag not in seen:
            seen.add(node.tag)
...
def analyze_explicit_blocks(source: str) -> ExplicitBlockPlan:
    """Analisa a projeção uma vez e determina se ela exige `blocks`."""

    text = str(source or "")
    parser = _TreeParser()
    parser.feed(text)
    parser.close()
    return ExplicitBlockPlan(text, parser.root, _explicit_reasons(parser.root))


def contains_semantic_entities(source: str) -> bool:
    """Shim legado; o pipeline novo usa `ExplicitBlockPlan.requires_blocks`."""

    return "tg-entity" in analyze_explicit_blocks(source).reasons
...
def _validate_button_text(value) -> None:
    """Restringe texto de RichMessageButton ao subconjunto aceito."""
...
def _inline_button(node: _Node) -> dict:
    """Único construtor para toda ocorrência canônica de `<tg-button>`."""
    attrs = node.attrs
    ...
    if typ == "url":
        ...
        raise ValueError(
            f"Tipo de botão Rich explícito desconhecido: {typ or '(vazio)'}."
        )
    return item
...
def _inline(value: _Node | str):
    ...
    if tag == "tg-emoji":
        return {
            "type": "custom_emoji",
            "custom_emoji_id": str(value.attrs.get("emoji-id") or ""),
            "alternative_text": _plain(inner),
        }
    if tag == "tg-time":
        return {
            "type": "date_time",
            "text": inner,
            "unix_time": int(value.attrs.get("unix") or 0),
            "date_time_format": str(value.attrs.get("format") or ""),
        }
    if tag == "tg-math":
    ...
    if tag == "tg-reference":
        return {
            "type": "reference",
            "text": inner,
            "name": str(value.attrs.get("name") or ""),
        }
    if tag == "tg-button":
    ...
    if tag == "tg-entity":
        ...
        if not field_name:
            raise ValueError(
                f"Tipo semântico Rich explícito desconhecido: {typ or '(vazio)'}."
            )
        ...
        if href.startswith("tg://user?id="):
            ...
            return {
                ...
                "user": {
                    "id": user_id,
                    "is_bot": False,
                    "first_name": _plain(inner) or "Telegram user",
                },
            }
...
def _caption(node: _Node | None):
    ...
    return {
        ...
        "credit": (credit[0] if len(credit) == 1 else credit) or None,
    }
...
def _media_block(node: _Node, media_by_id: dict[str, object]) -> dict:
    ...
    if not match:
        raise ValueError(
            "Entidade Rich explícita exige que a mídia use referência tg:// associada."
        )
    ...
    if isinstance(media, InputMediaPhoto):
        typ, field_name = "photo", "photo"
    elif isinstance(media, InputMediaAnimation):
        typ, field_name = "animation", "animation"
    elif isinstance(media, InputMediaVideo):
        typ, field_name = "video", "video"
    elif isinstance(media, InputMediaVoiceNote):
        typ, field_name = "voice_note", "voice_note"
    elif isinstance(media, InputMediaAudio):
        typ, field_name = "audio", "audio"
    elif isinstance(media, InputMediaDocument):
        typ, field_name = "document", "document"
    else:
...
def _child_blocks(node: _Node, media_by_id: dict[str, object]) -> list[dict]:
    ...
    for child in node.children:
        ...
        if child.tag in {
            ...
            "code", "mark", "sub", "sup", "a", "tg-spoiler", "tg-emoji",
            "tg-time", "tg-math", "tg-reference", "tg-entity", "tg-button",
            "span",
        }:
            loose.append(child)
            continue
...
def _block(node: _Node, media_by_id: dict[str, object]) -> list[dict]:
    tag = node.tag
    if tag == "p":
        return [{"type": "paragraph", "text": _inline_children(node)}]
    if re.fullmatch(r"h[1-6]", tag):
        return [{"type": "heading", "text": _inline_children(node), "size": int(tag[1])}]
    if tag == "footer":
        return [{"type": "footer", "text": _inline_children(node)}]
    if tag == "hr":
        return [{"type": "divider"}]
    if tag == "pre":
        code = next(
            (x for x in node.children if isinstance(x, _Node) and x.tag == "code"),
            None,
        )
        target = code or node
        language = ""
        ...
        return [
            {
                "type": "pre",
                "text": _plain(_inline_children(target)),
    ...
    if tag == "tg-map":
        lat = node.attrs.get("lat", node.attrs.get("latitude"))
        lon = node.attrs.get("long", node.attrs.get("longitude"))
        ...
        for name in ("zoom", "width", "height"):
            if node.attrs.get(name) not in (None, False, ""):
                block[name] = int(node.attrs[name])
        return [block]
    if tag == "tg-math-block":
        return [
            {
                "type": "mathematical_expression",
                "expression": _plain(_inline_children(node)),
            }
        ]
        ...
        return [
            {
                "type": "buttons",
                "buttons": buttons,
                "align": str(node.attrs.get("align"))
                if node.attrs.get("align") not in (None, False, "")
                else None,
            }
        ]
    if tag == "figure":
        media = next(
            (
                ...
                and x.tag in {"img", "video", "audio", "tg-document", "tg-map"}
            ),
            None,
        )
        if media is None:
            raise ValueError("<figure> Rich sem bloco de mídia ou mapa.")
        block = _block(media, media_by_id)[0]
        caption_node = next(
            (
                x
                for x in node.children
                if isinstance(x, _Node) and x.tag == "figcaption"
            ),
            None,
        )
    ...
    if tag in {"blockquote", "aside"}:
        cite = next(
            (x for x in node.children if isinstance(x, _Node) and x.tag == "cite"),
            None,
        )
        ...
        if tag == "aside" or "expandable" in node.attrs:
            body_holder = _Node(
                ...
                children=[
                    x
                    for x in node.children
                    if x is not cite and not (isinstance(x, str) and not x.strip())
                ],
            )
        ...
        return [
            {
                "type": "blockquote",
                "blocks": _child_blocks(node, media_by_id),
                "credit": credit,
            }
        ]
    if tag == "details":
        summary = next(
            (
                x
                for x in node.children
                if isinstance(x, _Node) and x.tag == "summary"
            ),
            None,
        )
        return [
            {
                "type": "details",
                "summary": _inline_children(summary) if summary else "",
                "blocks": _child_blocks(node, media_by_id),
                "is_open": True if "open" in node.attrs else None,
            }
        ]
    if tag in {"ul", "ol"}:
        items = []
        for li in (
            x for x in node.children if isinstance(x, _Node) and x.tag == "li"
        ):
            checkbox = next(
                (
                    x
            ...
            if checkbox:
                item.update(
                    {
                        "has_checkbox": True,
                        "is_checked": True if "checked" in checkbox.attrs else None,
                    }
                )
            if tag == "ol":
                if li.attrs.get("value") not in (None, False, ""):
                    item["value"] = int(li.attrs["value"])
                if li.attrs.get("type") not in (None, False, ""):
                    item["type"] = str(li.attrs["type"])
            items.append(item)
    ...
    if tag == "table":
        caption_node = next(
            (
                x
                for x in node.children
                if isinstance(x, _Node) and x.tag == "caption"
            ),
            None,
        )
        rows = []
        for tr in (
            x for x in node.children if isinstance(x, _Node) and x.tag == "tr"
        ):
            row = []
            for cell in (
                x
                for x in tr.children
                if isinstance(x, _Node) and x.tag in {"td", "th"}
            ):
                value = {
                    "text": _inline_children(cell),
                ...
                for name in ("colspan", "rowspan"):
                    if cell.attrs.get(name) not in (None, False, ""):
                        value[name] = int(cell.attrs[name])
                row.append(value)
            rows.append(row)
        return [
            {
                "type": "table",
                "cells": rows,
                "caption": _inline_children(caption_node) if caption_node else None,
                "is_bordered": True if "bordered" in node.attrs else None,
                "is_striped": True if "striped" in node.attrs else None,
                "is_compact": True if "compact" in node.attrs else None,
            }
        ]
    if tag in {"tg-collage", "tg-slideshow"}:
        ...
        return [
            {
                "type": "collage" if tag == "tg-collage" else "slideshow",
                "blocks": _child_blocks(node, media_by_id),
                "caption": _caption(caption_node),
            }
        ]
...
def compile_explicit_blocks(
    plan: ExplicitBlockPlan,
    media_by_id: dict[str, object],
) -> list[InputRichBlockUnion]:
    """Compila exatamente a árvore que determinou a estratégia de entrega."""
    blocks = _child_blocks(plan.root, media_by_id)
    if not blocks:
...
def compile_semantic_blocks(
    ...
    """Compatibilidade para consumidores que ainda chamam o compilador direto."""
    return compile_explicit_blocks(analyze_explicit_blocks(source), media_by_id)
