...
class _TreeParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs) -> None:
        node = _Node(
            tag.lower(),
            {name.lower(): (value if value is not None else True) for name, value in attrs},
        )
...
def _inline(value: _Node | str):
    ...
    if tag == "tg-button":
        return {"type": "button", "button": _build_button(value)}
    if tag == "tg-entity":
...
def _child_blocks(node: _Node, media_by_id: dict[str, object]) -> list[dict]:
    ...
    for child in node.children:
        ...
        if child.tag in {
            ...
            "tg-time", "tg-reference", "tg-entity", "tg-button", "span",
        }:
            loose.append(child)
            continue
        flush()
...
def _block(node: _Node, media_by_id: dict[str, object]) -> list[dict]:
    ...
    if tag == "figure":
        ...
        caption_node = next(
            (x for x in node.children if isinstance(x, _Node) and x.tag == "figcaption"),
            None,
        )
    ...
    if tag == "details":
        summary = next(
            (x for x in node.children if isinstance(x, _Node) and x.tag == "summary"),
            None,
        )
        return [{
            "type": "details",
    ...
    if tag == "table":
        caption_node = next(
            (x for x in node.children if isinstance(x, _Node) and x.tag == "caption"),
            None,
        )
        rows = []
        for tr in (
