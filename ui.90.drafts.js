...
function paintAutosave(pending){if(!autosaveEl)return;if(saving){autosaveEl.textContent=lastSavedAt?'salvando… · último '+stamp(lastSavedAt):'salvando…';return}if(lastSavedAt){autosaveEl.textContent=(pending?'alterado · saved ':'saved ')+stamp(lastSavedAt);return}autosaveEl.textContent=pending?'autosave pendente':'autosave aguardando'}
