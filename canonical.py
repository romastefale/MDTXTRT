...
def _inline_telegraph(text: str, degradations: list[str]) -> str:
    # Telegram-only semantic tags: keep readable content, never silently claim parity.
    ...
    # Inline formula: Telegraph has no math renderer; preserve source visibly.
    def formula(match: re.Match) -> str:
        degradations.append("Fórmulas LaTeX são preservadas como código porque o Telegraph não possui bloco matemático nativo.")
        return hold(f"<code>{html.escape(match.group(1))}</code>")

    escaped = re.sub(r"(?<!\$)\$([^\n$]+)\$(?!\$)", formula, escaped)
    escaped = re.sub(r"`([^`\n]+)`", lambda m: hold(f"<code>{html.escape(m.group(1))}</code>"), escaped)
    ...
    escaped = re.sub(r"&lt;(?:sub|sup)&gt;(.*?)&lt;/(?:sub|sup)&gt;", r"\1", escaped, flags=re.I | re.S)
...
def markdown_to_telegraph(source: str) -> TelegraphProjection:
    ...
    if re.search(r"(?m)^#{1,2}\s|^#{5,6}\s", text):
        degradations.append("Telegraph só possui h3/h4; H1-H2 e H5-H6 são mapeados deterministicamente para esses níveis.")
    if re.search(r"(?m)^\s*\|.*\|\s*$", text):
        degradations.append("Tabelas Rich/GFM são preservadas como bloco preformatado no Telegraph.")
    if "<details" in text.lower():
        degradations.append("Blocos <details> não existem no Telegraph; resumo e corpo são publicados expandidos.")
    if re.search(r"<tg-(?:map|button|collage|slideshow|reference)", text, re.I):
        degradations.append("Estruturas exclusivas do Telegram Rich são convertidas para uma representação textual/compatível no Telegraph.")
    if LOCAL_MEDIA_RE.search(text):
        degradations.append("Mídia de upload local é temporária e não pode ser persistida pelo Telegraph; apenas a legenda é mantida.")
    ...
    while i < len(lines):
        ...
        if media:
            ...
            if kind == "image":
                ...
                degradations.append("Áudio/documento Rich por URL vira link no Telegraph, que não oferece bloco equivalente confiável.")
                parts.append(f'<p><a href="{safe_url}">{html.escape(caption or "Abrir mídia")}</a></p>')
                i += 1
                continue
            if caption:
                parts.append(f"<figure>{node}<figcaption>{html.escape(caption)}</figcaption></figure>")
            else:
                parts.append(node)
            i += 1
            continue
        ...
        map_match = re.search(r'<tg-map\s+[^>]*lat="([^"]+)"[^>]*long="([^"]+)"[^>]*/?>', stripped, re.I)
        if map_match:
            flush_para()
            lat, lon = map_match.groups()
            url = f"https://maps.google.com/?q={lat},{lon}"
            parts.append(f'<p><a href="{html.escape(url, quote=True)}">Mapa: {html.escape(lat)}, {html.escape(lon)}</a></p>')
            i += 1
            continue

        # Button rows: retain labels/URLs as links.
        if stripped.lower().startswith("<tg-button-row"):
            flush_para()
            block: list[str] = []
            while i < len(lines):
                block.append(lines[i])
                if "</tg-button-row>" in lines[i].lower():
                    i += 1
                    break
                i += 1
            blob = "\n".join(block)
            ...
            for attrs, label in buttons:
                ...
                if url_match:
                    rendered.append(f'<a href="{html.escape(url_match.group(1), quote=True)}">{html.escape(label)}</a>')
                else:
        ...
        # Details: publish expanded.
        if stripped.lower().startswith("<details"):
            flush_para()
            block: list[str] = []
            while i < len(lines):
                block.append(lines[i])
                if "</details>" in lines[i].lower():
                    i += 1
                    break
                i += 1
            blob = "\n".join(block)
            summary = re.search(r"<summary>(.*?)</summary>", blob, re.I | re.S)
            body = re.sub(r"</?details[^>]*>|</?summary[^>]*>", "", blob, flags=re.I)
            if summary:
                parts.append(f"<h4>{_inline_telegraph(summary.group(1), degradations)}</h4>")
            body = re.sub(r"<summary>.*?</summary>", "", body, flags=re.I | re.S).strip()
            if body:
                parts.append(f"<p>{_inline_telegraph(body, degradations)}</p>")
            continue
        ...
        if re.match(r"^(---+|\*\*\*+)$", stripped):
            flush_para(); parts.append("<hr>"); i += 1; continue
        ...
        if heading:
            ...
            parts.append(f"<{tag}>{_inline_telegraph(heading.group(2), degradations)}</{tag}>")
            i += 1
            continue
        ...
        if stripped.startswith(">"):
            ...
            while i < len(lines) and (lines[i].strip().startswith(">") or not lines[i].strip()):
                ...
                if current.startswith(">"):
                    ...
                    quote.append("")
                i += 1
            parts.append(f"<blockquote>{_inline_telegraph(' '.join(quote).strip(), degradations)}</blockquote>")
            continue
        ...
        if list_match:
            ...
            while i < len(lines):
                ...
                if task and not ordered:
                    ...
                    items.append(f"<li>{mark} {_inline_telegraph(task.group(2), degradations)}</li>")
                elif ul and not ordered:
                    items.append(f"<li>{_inline_telegraph(ul.group(1), degradations)}</li>")
                elif ol and ordered:
                    items.append(f"<li>{_inline_telegraph(ol.group(1), degradations)}</li>")
                else:
                    break
                i += 1
        ...
        # Strip remaining Telegram-only wrapper tags while retaining text.
        cleaned = re.sub(r"</?(?:tg-collage|tg-slideshow|footer)[^>]*>", "", stripped, flags=re.I)
        para.append(cleaned)
        i += 1

    flush_para()
    return TelegraphProjection("".join(parts).strip() or "<p></p>", _unique(degradations))
